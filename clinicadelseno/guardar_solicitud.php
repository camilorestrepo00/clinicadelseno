...
if (strcasecmp($tipo_solicitud, 'Certificado') === 0) {
    ...
    $sql = "INSERT INTO solicitudes (numero_cedula, tipo_solicitud, tipo_certificado, fecha_inicio, hora_inicio, fecha_fin, hora_salida, motivo, archivo, fecha_solicitud, estado_jefe, estado_admin) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
    $stmt = mysqli_prepare($con, $sql);
    if (!$stmt) {
    ...
    mysqli_stmt_bind_param($stmt, "ssssssssssss", $numero_cedula, $tipo_solicitud, $tipo_certificado, $fecha_inicio, $hora_inicio, $fecha_fin, $hora_salida, $motivo, $archivo_nombre, $fecha_solicitud, $estado_jefe, $estado_admin);
    $exec_ok = mysqli_stmt_execute($stmt);
    $last_error = mysqli_error($con);
    if (!$exec_ok || mysqli_errno($con)) {
